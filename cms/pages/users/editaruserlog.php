...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($username_err) && empty($password_err)){
        $sql = "SELECT id, username,tipo, password FROM users WHERE id = :id";
                            ...
                            if($stmt = $pdo->prepare($sql)){
                                ...
                                if($stmt->execute()){
                                    if($stmt->rowCount() == 1){
                                        ...
                                        $aux = "OIIIIIIIIIIIII!!!!!!!!!!!!!!";
                                        $INSTRUCAO = $pdo->prepare('UPDATE users SET username = :username  WHERE id = :id');
                                        $INSTRUCAO->bindParam(':username',$usernameNew ,':id', $_SESSION['id'], PDO::PARAM_INT);
                                        $INSTRUCAO->execute();
                            ...
                            <div class="col-md-6 col-lg-12 d-flex align-items-center">
                                <div class="card-body p-4 p-lg-5 text-black">
                                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                                        <h5 class="fw-normal mb-3 pb-3" style="letter-spacing: 1px;">Alterar Nome de Utilizador</h5>
                                        ...
                                        $aux
                                        <label class="form-label" for="form3Example4"><?php echo $aux; ?> </label>
