...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(empty(trim($_POST["softskills"]))){
        $softskill_err = "Please fill softskills.";
    }  
        ...
        if(empty($softskills_err)) {
            
            $sql = "UPDATE softskills SET softskill=:softskill WHERE id=:id";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':softskill', $_POST['softskill'], PDO::PARAM_STR);
            $stmt->bindParam(':id', $_GET['id'], PDO::PARAM_INT);
                    ...
                    <div class="row g-0">
                        <div class="col-md-6 col-lg-12 d-flex align-items-center">
                            <div class="card-body p-4 p-lg-5 text-black">
                                <button onclick="window.location.href='listarSoftSkills.php'" class="btn btn-primary w-25 mb-5">
                                    Voltar
                                </button>

                                <form action="editarSoftSkills.php?id=<?php echo $_GET['id']; ?>" method="post">
                                    <h5 class="fw-normal mb-3 pb-3 letterSp">Edit User</h5>
                                    
                                    <!-- username input -->
                                    <div class="form-outline mb-4">
                                        <input type="text" name="softskill" class="form-control form-control-lg <?php echo (!empty($texto_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $inicio['softskill']; ?>" required>
                                        <label class="form-label" for="form3Example3">SoftSkill</label>
                                    </div>
