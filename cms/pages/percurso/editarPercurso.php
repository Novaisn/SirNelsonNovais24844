...
if(isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM percursoacademico WHERE id=:id";
...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Check if the username that was submitted, is already taken by another user
        ...
        if(empty($nome_err) && empty($descricao_err)) {
            ...
            $sql = "UPDATE percursoacademico SET ano=:ano, descricao=:descricao WHERE id=:id";
            $stmt = $pdo->prepare($sql);
                    ...
                    <div class="row g-0">
                        <div class="col-md-6 col-lg-12 d-flex align-items-center">
                            <div class="card-body p-4 p-lg-5 text-black">
                                <button onclick="window.location.href='listarPercurso.php'" class="btn btn-primary w-25 mb-5">
                                    Voltar
                                </button>

                                <form action="editarPercurso.php?id=<?php echo $_GET['id']; ?>" method="post">
                                    <h5 class="fw-normal mb-3 pb-3 letterSp">Editar Percurso</h5>
                                    
                                    <!-- username input -->
                                    <div class="form-outline mb-4">
                                        <input type="number" min="0" name="ano" class="form-control form-control-lg <?php echo (!empty($ano_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $inicio['ano']; ?>" required>
